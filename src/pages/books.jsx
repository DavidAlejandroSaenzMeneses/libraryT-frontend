...
import { BookOpenIcon, PencilAltIcon, ChevronDoubleUpIcon, ChevronDoubleDownIcon, PlusCircleIcon } from '@heroicons/react/outline';
        ...
        <div className="py-4 px-6 h-full">
            ...
            <div className="data-records block p-2 bg-white w-full rounded-md shadow-lg h-xl justify-center">
                <div className="flex justify-between p-3">
                    ...
                    <div></div>
                    <div>
                        <Link to={"/books/create/"} className="flex items-center w-32 border-2 border-gray-200 p-2 text-gray-700 rounded-md my-1 bg-gray-100 transition duration-500 truncate">
                            <PlusCircleIcon className="h5 w-5 mr-1" />Añadir Libro
                        </Link>
                    </div>
                </div>
                        ...
                        books.map((book, i) => {
                            return (
                                <div className="book-list-item flex h-sm p-2 my-2 h-60 bg-gray-100" key={'itemBook' + i}>
                                    <div className="book-image w-1/2 md:1/3 lg:w-1/5">
                                        <div className="flex h-60 w-40 items-center">
                                            {book.image !== null ? (
                                                <img src={urlBase + '/books/get-image/' + book.image} alt={book.title} className="object-scale-down h-48 w-96" />
                                            ) : (
                                                <img src={defaultPicture} alt="book" className="object-contain h-48 w-96" />
                                            )}
                                        </div>
                                    </div>
                                    <div className="book-info justify-start items-center hidden md:flex lg:w-3/5">
                                        <div className="block w-full">
                                            ...
                                            <div className="h-40">
                                                <div className="overflow-hidden h-32 text-ellipsis text-justify w-44 lg:w-full">{book.prologue}
                                                </div>
                                            </div>
                                            {book.loan_book ? (
                                    ...
                                    <div className="w-1/2 md:w-1/3 lg:w-1/5">
                                        ...
                                        <div className="flex items-center justify-center h-4/5">
                                            <div>
                                                <Link to={"/book/edit/" + book.id_book} className="flex items-center w-28 border-2 border-yellow-400 p-2 text-gray-700 rounded-md my-1 bg-gray-100 hover:bg-yellow-500 hover:text-white transition duration-500">
                                                    <PencilAltIcon className="h-5 w-5" /> Editar
                                                </Link>
                                                {book.loan_book ? (
                                                    <button className="flex items-center w-28 border-2 border-purple-400 p-2 text-gray-700 rounded-md my-1 bg-gray-100 hover:bg-purple-500 hover:text-white transition duration-500" onClick={closeLoan} value={book.id_book}>
                                                        <ChevronDoubleDownIcon className="h5 w-5" />Devolucion
                                                    </button>
                                                ) : (
                                                    <Link to={"/loan/create/" + book.id_book} className="flex items-center w-28 border-2 border-green-400 p-2 text-gray-700 rounded-md my-1 bg-gray-100 hover:bg-green-500 hover:text-white transition duration-500">
